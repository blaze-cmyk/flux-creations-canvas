...
export default function SpacesProjects() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      {/* Hero section */}
      <div className="relative mx-6 mt-4 rounded-2xl overflow-hidden" style={{ height: 380 }}>
        {/* Video background */}
        <video
          autoPlay
          muted
        ...
        <div className="relative z-10 h-full flex">
          ...
          <div className="flex flex-col justify-end p-10 max-w-md">
            <h1 className="text-5xl font-bold text-foreground mb-3 leading-tight">Spaces</h1>
            <p className="text-base text-muted-foreground mb-1">Start from scratch</p>
            <p className="text-sm text-muted-foreground/70 mb-5">Create a new space and start collaborating.</p>
            <button
              onClick={createProject}
              className="flex items-center gap-2 bg-foreground text-background px-5 py-2.5 rounded-lg text-sm font-medium hover:opacity-90 transition-opacity w-fit"
            >
          ...
          {/* Right side floating nodes preview */}
          <div className="flex-1 relative hidden lg:flex items-center justify-center">
            {/* Floating prompt card */}
            <div className="absolute top-16 right-48 bg-node/90 backdrop-blur border border-node-border rounded-xl p-4 max-w-[200px] shadow-2xl">
            ...
            {/* Floating image */}
            <div className="absolute bottom-12 right-64 w-28 h-28 rounded-xl overflow-hidden border-2 border-primary/30 shadow-2xl">
              <img src={spacesHeroImage} alt="" className="w-full h-full object-cover" />
            </div>

            {/* Floating action card */}
            <div className="absolute top-10 right-8 bg-node/90 backdrop-blur border border-node-border rounded-xl p-4 max-w-[220px] shadow-2xl">
              <p className="text-xs text-muted-foreground leading-relaxed">
            ...
            <svg className="absolute inset-0 w-full h-full pointer-events-none" xmlns="http://www.w3.org/2000/svg">
              <path d="M 200 180 C 250 220, 280 260, 220 300" stroke="hsl(var(--primary))" strokeWidth="2" fill="none" opacity="0.4" />
              <path d="M 320 120 C 350 160, 300 200, 260 250" stroke="hsl(var(--accent))" strokeWidth="2" fill="none" opacity="0.3" />
              <circle cx="200" cy="180" r="4" fill="hsl(var(--primary))" opacity="0.6" />
              <circle cx="220" cy="300" r="4" fill="hsl(var(--primary))" opacity="0.6" />
              <circle cx="320" cy="120" r="4" fill="hsl(var(--accent))" opacity="0.4" />
            </svg>
          </div>
        </div>
      </div>
