...
export default function MarketingStudioProject() {
  ...
  // Apply a DB row to the local store (used by both hydration and realtime).
  const applyRow = (projectId: string, row: any) => {
  ...
  // One-time hydration of generations from DB on mount/project switch.
  useEffect(() => {
    if (!project) return;
    const projectId = project.id;
    // Cleanup non-UUID placeholders from old buggy versions.
    ...
    (async () => {
      ...
      for (const row of data) applyRow(projectId, row);
    })();
    return () => { cancelled = true; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [project?.id]);

  // Realtime: instant queued → running → done updates with auto-reconnect.
  useRealtimeTable(
    {
      table: 'ms_generations',
      filter: project ? { column: 'project_id', value: project.id } : undefined,
      channelKey: `ms_gen:${project?.id ?? 'none'}`,
      enabled: !!project,
    },
    {
      onUpsert: (row) => project && applyRow(project.id, row),
    },
  );
